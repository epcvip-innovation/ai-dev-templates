...
### Classifier vs Review Jobs

| Component | Cost | Requires API Key | What it does |
|-----------|------|-----------------|--------------|
| Classifier (tier labeling) | Free | No | Bash + jq — labels PRs by risk tier |
| Security review | ~$0.10-0.30/PR | Yes | Anthropic API scans diff for vulnerabilities |
| QA review (browser) | ~$0.50-1.50/PR | Yes | Anthropic API + Playwright tests the running app |

### When API Reviews Are Worth It

The classifier alone gives you tier visibility and merge gating — no API key, no cost. Whether to add the API-based review jobs depends on project stakes:

- **Classifier-only is sufficient** if you already run `/local-review` reliably before PRs. The classifier ensures tier labels are visible and can block merges on high-risk paths. The actual review happens locally via your Claude subscription.
- **API reviews pay for themselves** when a missed review could cost hundreds or thousands — production auth code, payment flows, PII handling. Automated CI reviews catch what humans forget to run locally.
- **The decision depends on project stakes, not project size.** A solo project handling payments needs API reviews more than a large team project that only serves docs.

### Per-PR Cost With Gating
...
- **Claude Pro/Max/Team subscription** covers local Claude Code usage — skills, plugins, `/local-review`. This is what you already pay for.
- **Anthropic API** (console.anthropic.com) is pay-per-token, billed separately. CI review jobs (`security-review`, `qa-review`) use this.
- **The classifier uses neither** — it's pure bash + jq, runs on GitHub Actions minutes only.

## Where to Start

Best suited for repos with distinct auth/permissions code AND a web UI — e.g., an admin panel (RBAC matrix), a docs site (per-doc access control), or a dashboard (role-based visibility). These have the highest ratio of mixed-risk PRs — a single sprint typically includes both docs-only and auth-touching changes, so risk gating saves the most CI spend there.
